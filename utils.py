...
def calc_psnr(predictions, targets):
    ...
    #     metric.append(val)
    # return metric
    # 假设 predictions 和 targets 是列表或张量，转换为批量张量
    if isinstance(predictions, list) or isinstance(predictions, tuple):
        predictions = torch.stack(predictions, dim=0)
        targets = torch.stack(targets, dim=0)
    
    predictions = as_img_array(predictions)
    targets = as_img_array(targets)
    
    mse = torch.mean((predictions - targets) ** 2.0, dim=(1, 2, 3), dtype=torch.float32)
...
def calc_msssim(predictions, targets):
    ...
    if isinstance(predictions, list):
        predictions = torch.stack(predictions, dim=0)  # (batch_size, 3, H, W)
        targets = torch.stack(targets, dim=0)          # (batch_size, 3, H, W)
    
    predictions = as_img_array(predictions)  # 转换为 [0, 255] 范围
    targets = as_img_array(targets)
    
    ssim_val = ssim(predictions, targets, data_range=255, size_average=False)  # 每个样本的 SSIM
    return ssim_val.tolist()  # 返回列表以保持一致性
...
def calc_ssim(predictions, targets):
    ...
    ssim_val = ssim(predictions, targets, data_range=255, size_average=False)
