...
def evaluate(net: torch.nn.Module, dataloader: Iterable, 
    ...
    attack = FGSM_REG(net, 128./255., 2./255., min_val=0, max_val=1, max_iters=8)
    
    with torch.no_grad():
        for batch_idx, (imgs, targets) in enumerate(dataloader):
            imgs, bm_pos = imgs  # 解包元组，imgs 是图像张量
            ...
            if if_attack:
                ...
                imgs = per_data
                psnr_values = calc_psnr(imgs.unbind(0), original_imgs.unbind(0))
                ssim_values = calc_ssim(imgs.unbind(0), original_imgs.unbind(0))
            else:
                psnr_values = [float('nan')] * imgs.size(0)
                ssim_values = [float('nan')] * imgs.size(0)
            ...
            if batch_idx % print_freq == 0:
                print('Test %d/%d: [loss: %.4f] [acc1: %.3f/100] [psnr: %.2f] [ssim: %.4f]' 
...
def train_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    attack = FGSM_REG(model, 128./255., 2./255., min_val=0, max_val=1, max_iters=4)
    ...
    for data_iter_step, (samples, targets) in enumerate(data_loader):    
        ...
        samples, bm_pos = samples  # 解包元组，samples 是图像张量
        targets = targets.to(device, non_blocking=True)
        original_samples = samples.clone().to(device, non_blocking=True)  # 图像张量
        ...
        if if_attack:
            ...
            noise = per_data - samples  # 计算扰动
            print(f"Epoch {epoch}, Batch {data_iter_step}: Max noise amplitude = {torch.max(torch.abs(noise)):.6f}")
            samples = per_data
            batch_size = samples.size(0)
            psnr_values = calc_psnr(samples.unbind(0), original_samples.unbind(0))
            ssim_values = calc_ssim(samples.unbind(0), original_samples.unbind(0))
        else:
            batch_size = samples.size(0)
            psnr_values = [float('nan')] * batch_size
            ssim_values = [float('nan')] * batch_size
        ...
        if data_iter_step % print_freq == 0:
            print('Epoch:[%d] %d/%d: [loss: %.3f] [acc1: %.3f /100] [lr: %.3e] [psnr: %.2f] [ssim: %.4f]' 
